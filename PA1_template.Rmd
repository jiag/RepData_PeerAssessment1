...
```{r, echo=FALSE}
setwd("~/DataScience//Reproducible_research/RepData_PeerAssessment1/")
```
...
median_steps
```
The mean steps per day is about **`r mean_steps`** and the median is about **`r median_steps`**.
...
max_intv
```
According to above analysis, the **`r max_intv`th** interval contains the maximum number of steps.
...
sum_na
```
We observed that there are lots of missing data in the dataset and the number of NA in the dataset is **`r sum_na`**. In the following we will replace the missing step values(NA) by the averaged number of steps for the corresponding interval accross all days.
```{r}
...
impute_median_steps
```
The mean of steps is **`r impute_mean_steps`** and the median is **`r impute_median_steps`**. From above analysis we observe that compare to the original dataset the mean of total number of steps per day stays the same but the median increases a little bit with the imputed data. And compare the histogram of the original and imputed data, we find that imputing missing data especially increases the number of days which has total number of steps between 10000 and 11000. 
